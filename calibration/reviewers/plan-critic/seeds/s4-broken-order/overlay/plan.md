...
Using the fake clock helper from s3, write the failing rotation test: presenting
a token with <5m validity returns a NEW token and the old one stops working
(both asserted, name tagged @spec:auth-refresh). Implement rotation in the
refresh handler with atomic swap; green.
...
A fake clock helper so expiry windows are testable without sleeps. It is
created here, after s1/s2 land. No behavior of its own — rigging for s1/s2.
