# NOTICE — third-party material in specflow

specflow synthesizes ideas and, where licenses permit, text from five sources
...
- `plugin/skills/specflow-brainstorm/SKILL.md` (← skills/brainstorming)
- `plugin/skills/specflow-plan/SKILL.md` (← skills/writing-plans)
- `plugin/skills/specflow-implement/SKILL.md` (← skills/test-driven-development)

From **mattpocock/skills** — `LICENSES/mattpocock-skills-MIT.txt`:

- `plugin/skills/specflow-brainstorm/SKILL.md` (← grill-me: one-question-with-recommendation interview style)
- `plugin/skills/specflow-implement/SKILL.md` (← tdd: red/green loop discipline)

From **no-mistakes / axi family** (kunchenguid) — `LICENSES/no-mistakes-MIT.txt`:

- `plugin/skills/specflow-ship/SKILL.md` (← no-mistakes: validate-then-ship step sequence, re-owned)
...
- `plugin/skills/specflow-design/SKILL.md` — greenfield design direction. **Changes:** rewritten for specflow's stage contract (spec-scoped artifact, CLI write path, design gate); merged with a benoticed-derived process and a specflow-native interactive loop. No text copied verbatim.

## Concept-adapted — zero text

From **benoticed.co** `docs/ui/redesign-method.md` (private, same owner) — the redesign *process* (context phase → 2–3 distinct directions → converge → change-ladder) informed `plugin/skills/specflow-design/SKILL.md` conceptually; **no text was copied**, and the method was generalized to two modes (new-screen / amend-living-look) and data-shape anchoring.
...
New material written for specflow (no upstream text): `prompts/slicing-critic.md`,
`prompts/plan-critic.md`, `prompts/drift-reviewer.md`, `prompts/design-critic.md`,
`plugin/skills/specflow-decompose/SKILL.md`, `plugin/commands/specflow.md`, both hook
