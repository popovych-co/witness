...
**D142 — One rebase base, and it is the remote's.** Two halves. (prose) The ship skill's manual-conflict recipe rebases on `origin/<ship-branch>` after a fetch, matching `ship.ts:130-174` — following the current recipe recreates the stale base the CLI just refused. (behavior) `diffBase` resolves against `origin/<ship-branch>` for **all seven callers** (implement gate, ship, ship gate, `next`'s evidence hints, the evidence verbs, calibrate — `evidence.ts:141-146` is one home and stays one home), falling back to the local branch when no remote exists (D137's no-remote legality). Post-D137 this is exactly the cut point for every plan branch, so it is correct at implement time even with stale remote refs; it closes the ship-gate shape where a rebased worktree hands reviewers a sibling's merged work as the judged diff. The halves ship in different waves (see Release).
