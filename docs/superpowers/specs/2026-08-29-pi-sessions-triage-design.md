...
**D137 — Worktrees are cut from the fetched remote tip.** `start` runs `git fetch origin <ship-branch>`; the create path cuts new plan branches from `origin/<ship-branch>`, not the local ref (re-attach of an existing branch is unchanged — its base is history). Fetch failure with a remote configured refuses with the git text and remedy — no silent local fallback, which would quietly reintroduce the root. A repo with no remote keeps the local cut (divergence needs a remote to exist). This alone makes squash-merge permanently harmless: plan branches can never carry state commits again, so the reproduced conflict shape (E1) becomes unconstructible. The cut is deliberately decoupled from local main's health: a failed D138 preflight (conflict, dirty tree) prints its finding and does **not** block the cut — the origin-based branch is correct regardless of local main's state, which is the property that makes this the root fix rather than another discipline.

**D138 — Sync becomes automatic at the two natural moments.** The post-merge lazy stamp runs the sync sequence (pull --rebase + push) immediately after its state commit — the moment origin is known to have moved; `start` preflights sync before cutting. Failure is a printed finding, never a crash; both invocations are journaled. Sequencing: the sync core runs **outside** the stamp's lock and transaction (sync acquires the lock itself; the stamp's txn must be durable before any rebase touches the tree). Push rejection is classified: a protected ship-branch is named as the likely cause rather than a bare `push failed` tail. Local main converges without the human knowing `sync` exists. Manual `witness sync` survives unchanged. **Residual, recorded not solved:** witness's state model requires push access to `<ship-branch>` — a repo whose main is protected (PR-only) cannot receive state commits at all, today or after this row; that is the A2 shape this triage rejected, revisited only on field demand. D139's finding plus this classification make the incompatibility visible instead of silent.

**D139 — Divergence is visible state.** `check` gains an ahead/behind/diverged finding against `origin/<ship-branch>` naming `witness sync` as remedy; the dashboard prints the same one-liner (one computation, two renderers — the D101 boundary). The comparison is best-effort and silent when the network is absent, on the registry-query precedent (D103): an offline machine reports nothing rather than a complaint. (A user's `pull.ff=only` + hand-run `git pull` can still fatal inside the brief ahead-window; the check line is what explains it, and D138 keeps the window near zero.)
...
**D141 — Worktree removal is cwd-safe.** `removeWorktree` callers (lazy stamp, done-sweep, `clean`, `abandon`) skip a worktree containing the invoking process's cwd and print a relocation line instead; the skipped removal re-runs harmlessly from any other cwd (the done-sweep already re-checks). Kills `Working directory does not exist` at the source. **Residual, accepted:** a *different* session's cwd is unknowable (no cross-session registry), so a second terminal can still delete a first terminal's worktree; the reported ×3 were all self-deletions (SessionStart hook, own `next`), which this closes.

**D142 — One rebase base, and it is the remote's.** Two halves. (prose) The ship skill's manual-conflict recipe rebases on `origin/<ship-branch>` after a fetch, matching `ship.ts:130-174` — following the current recipe recreates the stale base the CLI just refused. (behavior) `diffBase` resolves to the **true cut point: the descendant of `merge-base(HEAD, <branch>)` and `merge-base(HEAD, origin/<branch>)`** — one rule for **all seven callers** (implement gate, ship, ship gate, `next`'s evidence hints, the evidence verbs, calibrate — `evidence.ts:141-146` is one home and stays one home). No remote → the local half alone (D137's no-remote legality); incomparable ancestries → the origin half (the post-D137 invariant). Why not origin alone: **reproduced by experiment** — a legacy branch cut from a state-carrying local main diffs its inherited `.witness/journal/*` against the origin merge-base, so reviewers would judge state files as the plan's change; the descendant rule is exact for legacy-contaminated, post-D137-clean, and behind-local cases alike. Closes the ship-gate shape where a rebased worktree hands reviewers a sibling's merged work as the judged diff. The halves ship in different waves (see Release).

### Dismantle the treadmill (B1 approved; interview stays one-question-at-a-time by explicit user choice)

**D143 — A bare affirmation selects the recommendation** (amends D127, whose "a bare affirmation explicitly not counting as a selection" this reverses for one case). When a decision block carries a `recommended` option, `y`/`ok`/`go` selects it; the `human-decision` entry records `selected: affirmation` so closure-by-nod stays distinguishable from a named selection in the journal. D127's actual guard survives intact: the agent still types the printed string byte-for-byte, never recomposed — only the selection source changes, and it binds to a CLI-computed, journaled recommendation (D121), never to anything the agent authored. **Exclusions — these require naming the option, never a nod:** an obligation-minting `--approve --override` (D122's ledger must not open on an "ok"), terminal acts (`abandon`, discarding exits), and trust grants (D154). Blocks without a recommendation keep the ask-again rule. **Authorship decides the semantics** — the rule splits on who computed the recommendation, not on the five-field shape (which the interview deliberately mimics, `witness-brainstorm/SKILL.md:36`): a **CLI-rendered** block (journaled `recommended` + `rule`) gets selection semantics as above; an **agent-authored** question (the brainstorm interview, the design converge step) treats an affirmation as a plain conversational acceptance of the stated recommendation — no selection entry, no journal claim, and D121's doctrine unbreached because nothing gate-consumed binds to a model-authored recommendation; D127's byte-for-byte rule still governs anything the agent then runs, and the recap's explicit-confirmation table remains the scope gate. The rule is **surface-independent**: one definition, applied by skill prose in chat today and by `drive`'s TTY prompt when D145 builds it. One rule amendment across the six skills + engine. **Residual, accepted:** this widens D121's named residual — `--approve` at a judgment stop can now be taken by affirmation; the block was rendered verbatim and the journaled `selected: affirmation` makes the cost measurable rather than arguable.
...
**D145 — `witness drive`: the CLI becomes the scheduler (own effort; amends D82).** D82's finding stands — fresh, exactly-scoped context per slice, never Task-tool subagents — but its implementation conscripted the human as spawner. Drive changes the spawner, not the economics: the CLI gains a second spawn surface (the session lane, beside D48's judgment-lane reviewer battery). New verb: loop `next` → green-path action → spawn a fresh **headless** session (declared harness's print mode — precedent `harness.ts:130-144`, the calibration worker; cwd = the action's `home`) → stream progress → repeat. Returns to the human terminal at: judgment stops (feature scope, design, ship, blocking finding, pin contradiction, fallback), refusals with no mechanical remedy, round bounds, spawn failure, **stages that are conversations** (brainstorm; the design stage's authoring/converge session — drive yields, a human chat session runs them), and **no progress** (the same `next:` line twice with nothing journaled between — the engine's own rule, moved into the loop that now owns it). Relay boundaries and cross-home handoffs become drive's spawns; the dispatch budget survives as spawn sizing (`stepsPerDispatch` default 3 unchanged — it was measured against context limits, D79; drive removes its human cost, not its value). All existing bound/budget/obligation machinery binds drive — autonomy cannot outspend the round bound. Decided here: mechanism, CLI ownership, stop set, and that orchestration lives in the CLI rather than skill prose (D128: prose is the most expensive surface). Verb surface, streaming format, timeout policy: its own brainstorm → plan.
...
**D154 — Trust is granted where a human decides, consumed where absent.** Headless gates block every untrusted per-criterion command (`allowlist.ts:21`, `!isTTY → blocked`) — the Aug 1 all-four-`ac-*`-fail false-negative. The naïve fix — prompt at approval time — rests on a false assumption: approvals are agent-typed (`decide.ts` contains no TTY read; D127 is the doctrine), so there is no prompt to give. Instead, trust rides the decision surface that already exists: when the artifact under decision carries untrusted `cmd:` criteria, the block lists those commands verbatim and renders **both** approve forms — plain `--approve` (tradeoff line: the listed commands stay blocked at headless gates) and `--approve --trust-cmds` — so trust is never the toll for approval; the human names the trusting form (a bare affirmation never grants trust — D143 exclusion), the agent types it byte-for-byte, and `decide` writes the grants to `allow.json`. For paths where no stop exists (green-path classes), a new `witness trust <id>` verb lists and grants an artifact's commands, and the headless block's refusal names it as remedy beside `WITNESS_TRUST_CMDS=1`. Grants are journaled. Also unifies the root asymmetry: trust list and runner config both resolve at the **primary root** (canon-single-home doctrine, D132 — a branch checkout cannot re-point what the repo trusts or runs).
...
- **Wave 1 — statement honesty and text, no outcome changes:** D139, D140, D142-prose, D146, D147, D148, D149, D150, D152, D153, D155 (doc row only), D156.
