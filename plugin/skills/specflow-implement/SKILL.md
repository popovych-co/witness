...
```bash
SPECFLOW="${SPECFLOW_BIN:-npx -y @specflow/cli@0.1.3}"
```
...
> 2. Run `${SPECFLOW_BIN:-npx -y @specflow/cli@0.1.3} test-evidence <plan-id> --phase red` from the worktree. It must record a genuine red. If it reports the test passes before implementation (`vacuous`), STOP and report — the test asserts nothing, or the behavior already exists.
