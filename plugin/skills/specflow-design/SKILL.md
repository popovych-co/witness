...
- The design gate **always stops** — the look is human judgment, same footing as ship. Render the gate output verbatim and print the exits: `specflow decide design <spec-id> --approve | --revise --note "…" | --stop`. **END YOUR TURN.** You never decide.
- **Re-entered after `--revise`** → `specflow decide design <spec-id> --show` reconstructs the verdict + note (findings anchor to `design#<id>` or `<spec-id> > ## Heading`). Re-author the HTML, re-run `specflow design`, re-gate. The 3-round bound is the CLI's — surface it, never fight it.
