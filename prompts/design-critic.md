...
2. **Spec coverage.** Every behavior the spec's `## Behavior` promises must be *visible and operable* in the look — the design-side twin of criterion↔step totality. A promised behavior with no element expressing it is **blocking** (anchor it as an omission scoped to the spec heading). A screen showing capabilities the spec does not describe is a note, not a block (scope creep surfaces at decompose).
